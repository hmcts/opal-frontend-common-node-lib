...
export class Helmet {
  ...
  public enableFor(app: express.Express, enabled: boolean): void {
    if (enabled) {
      ...
      const scriptSrc = [self, dynatraceDomain, "'sha512-+6WnXIl4mbFTCARd8N3COQmT3bJJmo32N8q8ZSQAIcU='"];

      if (this.developmentMode) {
        // Uncaught EvalError: Refused to evaluate a string as JavaScript because 'unsafe-eval'
        // is not an allowed source of script in the following Content Security Policy directive:
        // "script-src 'self' *.google-analytics.com 'sha512-+6WnXIl4mbFTCARd8N3COQmT3bJJmo32N8q8ZSQAIcU='".
